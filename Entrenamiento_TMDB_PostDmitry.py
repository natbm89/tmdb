...
if df is not None:
    ...
    if (df_cleaned['revenue'] > 0).any():
        ...
        print("Ojo: No hay valores de 'revenue' mayores que cero para aplicar escala logarítmica.")
    ...
    if (df_cleaned['budget'].dropna() > 0).any(): 
        ...
        print("Ojo: No hay valores de 'budget' mayores que cero para aplicar escala logarítmica.")
...
# --- PASO 3: Definir la Métrica de Éxito y Crear la Variable Objetivo ---
...
umbral_vote_count = df_cleaned['vote_count'].median() # Mediana del conteo de votos
umbral_popularity = df_cleaned['popularity'].median() # Mediana de la popularidad
...
# Creamos la variable 'exito' (1 para éxito, 0 para fracaso)
    ...
    print(f"Umbral de Conteo de Votos (mediana): {umbral_vote_count}")
    print(f"Umbral de Popularidad (mediana): {umbral_popularity}")
...
# Cuidado, recordar: Para las columnas de Top N (production_companies_TOP_X, etc.),
# se generarían muchas gráficas, si queremos analizarlas, tiene que ser individualmente
# o con un heatmap de correlación si son muchas.
...
# Repetimos para mayor consistencia:
...
f_pre_model['in_collection'] = df_pre_model['belong_to_collection'].notna()
...
# --- PASO 5: Entrenamiento y Evaluación del Modelo de Clasificación ---
